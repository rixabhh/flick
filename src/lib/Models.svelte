...
<section class="models">
  ...
  {#each models as model}
    <article>
      <div><strong>{model.name}</strong><p>{model.description}</p><span>{model.engine} · {model.language} · {size(model.size_bytes)}</span><div class="capabilities"><span class="capability local">On device</span><span class="capability">{languageBadge(model)}</span>{#if model.supports_translation}<span class="capability translation">English translation</span>{:else}<span class="capability muted">Transcription only</span>{/if}</div></div>
      {#if model.installed}
