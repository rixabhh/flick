<script>
  ...
  // Keep the picker broad enough for every verified local catalog family.
  ...
  const knownDictationLanguages = [
    ...
    ["nl", "Dutch"], ["ru", "Russian"], ["uk", "Ukrainian"], ["pl", "Polish"],
    ["tr", "Turkish"], ["vi", "Vietnamese"], ["th", "Thai"], ["id", "Indonesian"],
    ["cs", "Czech"], ["da", "Danish"], ["el", "Greek"], ["fa", "Persian"],
  ...
  const modelSupportsLanguage = (language) => language === "auto"
    ? localModelCapabilities?.supports_language_detection !== false
  ...
  <div id="settings-tabpanel" class="tab-content" role="tabpanel" aria-labelledby={`settings-tab-${activeTab}`} tabindex="-1">
    ...
    {:else if activeTab === "dictate"}
      <div class="panel-section animate-fade-in">
        ...
        <label class="setting-field"><span>Spoken language</span><select bind:value={config.dictation_language} onchange={() => updateConfig("dictation_language", config.dictation_language)}>{#each dictationLanguageOptions as [code, label]}<option value={code} disabled={config.dictation_provider === "local-whisper" && !modelSupportsLanguage(code)}>{label}</option>{/each}<option value="auto" disabled={config.dictation_provider === "local-whisper" && !modelSupportsLanguage("auto")}>Detect automatically</option></select>{#if config.dictation_provider === "local-whisper" && localModelCapabilities?.supported_languages?.length}<small>This model supports: {localModelCapabilities.supported_languages.join(", ")}. Unsupported languages and automatic detection are unavailable rather than silently passed to the engine.</small>{/if}</label>
        {#if config.dictation_provider === "local-whisper" && localModelCapabilities && !modelSupportsLanguage(config.dictation_language)}<div class="panel quick-card"><strong>Your spoken-language choice needs attention</strong><span class="text-secondary">The selected model cannot use {config.dictation_language === "auto" ? "automatic detection" : config.dictation_language}. Choose an enabled language before starting dictation.</span></div>{/if}
        {#if config.dictation_provider === "local-whisper"}{#if localModelCapabilities && !localModelCapabilities.supports_translation && config.dictation_translate_to_english}<div class="panel quick-card"><strong>Translation was turned on for an older model</strong><span class="text-secondary">Turn it off before dictating, or choose a verified model that supports English translation. Flick will not ask the selected model to do an unsupported task.</span></div>{/if}<div class="toggle-container"><div class="toggle-label"><span class="toggle-label-text">Translate speech to English</span><span class="toggle-label-desc">Available only with a verified compatible local model. The model picker shows which models transcribe only.</span></div><label class="toggle"><input type="checkbox" checked={config.dictation_translate_to_english} disabled={localModelCapabilities && !localModelCapabilities.supports_translation && !config.dictation_translate_to_english} onchange={() => updateConfig("dictation_translate_to_english", !config.dictation_translate_to_english)} /><span class="toggle-slider"></span></label></div>{:else}<div class="panel quick-card"><strong>Translation is unavailable in cloud mode</strong><span class="text-secondary">Compatible transcription APIs vary by model. Flick will not make a translation claim it cannot verify.</span></div>{/if}
